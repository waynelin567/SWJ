    ...
    def validate(
        ...
        for constraint_component in constraint_components:
            ...
            try:
                ...
                self.constraint_components.append(c)
        ...
        if not non_conformant:
            for f, v in focus_value_nodes.items():
                self.fvcs.add(FVC(self, f, v, self.constraint_components))
        return (not non_conformant), reports

class FVC:
    def __init__(self, shape: Shape, focus_node: URIRef, value_nodes: list, constraints: list):
        self.shape = shape
        self.focus_node = focus_node
        self.value_nodes = list(value_nodes)
        self.constraints = {} 
        for constraint in constraints:
            self.constraints[constraint] = None
    def __str__(self):
        return f"FVC for {str(self.shape)} with focus node {str(self.focus_node)}, value nodes: {str(self.value_nodes)}"
    def __eq__(self, other):
        if not isinstance(other, FVC):
            return False
        return self.focus_node == other.focus_node and set(self.value_nodes) == set(other.value_nodes)

    def __hash__(self):
        return hash(self.focus_node) 
