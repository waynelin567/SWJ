...
def validate_with_validator_recorded(
    ...
    **kwargs,
    ...
    try:
        ...
        ValidatorMgr().set_validator(validator=validator)
        ValidatorMgr().set_data_graph(loaded_dg)
...
class ValidatorMgr:
    ...
    _data_graph: GraphLike = None
    _ont_graph: GraphLike = None
    _ont_predicates: list = []
    _output_dir = None
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(ValidatorMgr, cls).__new__(cls)
        return cls._instance
    def set_validator(cls, validator: Validator):
        cls._validator = validator
    def set_data_graph(cls, data_graph: GraphLike):
        cls._data_graph = data_graph
    def set_ont_graph(cls, ont_graph: GraphLike):
        cls._ont_graph = ont_graph
        preds = set()
        for s, o in ont_graph.subject_objects(RDF_type):
            if o.endswith("Property"):
                preds.add(s)
        cls._ont_predicates = list(preds)
